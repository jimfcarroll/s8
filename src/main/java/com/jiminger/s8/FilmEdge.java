...
public class FilmEdge {
   ...
   /**
    * This method will cut a piece of the film edge at r,c (or the closest point along the edge line) with a total
    * length of 'extent'.
    */
   public FilmEdge edgePiece(final double x0r, final double x0c, final double extent, final boolean usedPruned) {
      final ai.kognition.pilecv4j.image.geometry.Point Xi = Utils.closest(new SimplePoint(x0r, x0c), edge);
   ...
   /**
    * This will return the two edges of the film such that the first element will be the edge that is closest to the
    * sprocket if the image is not reversed.
    */
   public static FilmEdge[] getEdges(final int filmLayout, final CvMat edgeImage, final byte edgeVal, final CvMat gradientRaster, final boolean prune)
         throws MinimizerException {
      List<Point> l1;
      List<Point> l2;
   ...
   public void writeEdge(final CvMat image, final byte overlayPixelValue) {
      writePoints(pixels, image, overlayPixelValue);
   }

   public void writePruned(final CvMat image, final byte overlayPixelValue) {
   ...
   private static void writePoints(final List<Point> points, final CvMat image, final byte overlayPixelValue) {
      final byte[] overlayPixel = new byte[] {overlayPixelValue};
      for(final Point p: points)
         image.put(p.y, p.x, overlayPixel);
   }

   private static List<Point> getTopEdge(final CvMat edgeImage, final byte edgeVal, final CvMat gradientRaster) {
   ...
   private static List<Point> getBottomEdge(final CvMat edgeImage, final byte edgeVal, final CvMat gradientRaster) {
   ...
   private static List<Point> getLeftEdge(final CvMat edgeImage, final byte edgeVal, final CvMat gradientRaster) {
   ...
   private static List<Point> getRightEdge(final CvMat edgeImage, final byte edgeVal, final CvMat gradientRaster) {
