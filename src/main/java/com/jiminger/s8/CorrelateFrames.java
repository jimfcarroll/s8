...
import com.jiminger.s8.internal.FilenameUtils;
...
public class CorrelateFrames {
    ...
    /** The main method. */
    public static void main(final String[] args) throws IOException, Correlate.CorrelateException,
        MinimizerException {
        ...
        while(iter.hasNext()) {
            ...
            if(!overloadedMatch(fs1, fs2, overloadedMatches)) {
                if(fs1.hasForcedMatchNext()) {
                    ...
                    final CvMat frameImage1 = loadLastImage(fs1);
                    ...
                    if(frameImage1 == null) {
                        ...
                        final CvMat[] frameImage2 = loadImages(fs2);
    ...
    static private CvMat[] loadImages(final FrameSet fs) {
        /*
        ...
        final CvMat[] frameImage = new CvMat[fs.numFrames];
        for(int i = 0; i < fs.numFrames; i++) {
            if(filenames[i] != null)
                frameImage[i] = CvMat.shallowCopy(Imgcodecs.imread(filenames[i]));
            else
                frameImage[i] = null;
        }
    ...
    static private CvMat loadLastImage(final FrameSet fs) {
        /*
         * Create an input stream from the specified file name to be used with the file decoding operator.
         */
        ...
        return CvMat.shallowCopy(Imgcodecs.imread(filename));
    }
